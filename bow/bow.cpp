...
double bow_get_starting_elapsed_time(void)
{
    double et;
...
int bow_resolve_filename(const char *logical_name, std::string& physical_name)
{
    int err = 0;
    ...
    if (boinc_is_standalone())
        ...
        err = boinc_resolve_filename_s(logical_name, physical_name);

#ifdef VERBOSE
    printf("File %s resolved as %s\n", logical_name, physical_name.c_str());
#endif
    return err;
}
