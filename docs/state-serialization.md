...
- **`SerializedGWNum` internals.** This doc treats it as "uint32 word count + words" on the wire (`file.cpp:37-41, 146-157`); what those words *mean* (the GWnum `gwserialize` representation) is covered at the binding level in `arithmetic-foundation.md` §6. Cross-FFT-*length* portability for the same modulus is resolved there (§9): a same-length load is the fast path, a **larger** transform re-radix-converts, a **smaller** one throws — so a `SerializedGWNum` checkpoint survives a reliable-mode FFT bump, which only grows the transform.
