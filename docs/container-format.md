...
- **Streaming reads are possible.** A file's index entry always appears *before* the first chunk of its stream, so a reader can unpack every file on the fly in a single sequential pass over the container data — no second pass, no seeking back. (The in-tree `FileContainer` reader is the lazy/seeking one in §2; the format itself permits single-read extraction.)
- **Append is also the repair strategy.** If PRST on restart refuses a file from the container (e.g. its MD5 check fails), it does **not** delete the old data or rebuild the container — that would be a costly operation with many failure modes. It simply appends a corrected copy, whose index entry overrides the bad one. This is sound because PRST re-reads all written data on restart, so a bad entry is always detected and superseded.
...
- **Later index entries override earlier ones.** A filename appearing in two index chunks resolves to the *last* (`open` resets and reassigns). That's the append mechanism — and the *repair* mechanism (§1): a refused/corrupt file is fixed by appending a good copy, never by rewriting. It also means a truncated/duplicated append can silently shadow good data; the offset/size validation and MD5 are the guards.
- **Offsets are bounded by the 16 MB chunk size.** `open` flags `offset > 16777215` as `INDEX_CORRUPTED` — a file's offset is *within* its stream's chunk addressing, not a global file position. Reading the offset as an absolute byte position into the `.pack` is wrong. The bound is a fundamental format limitation, not an arbitrary sanity check: a compressing codec doesn't support seeking, so reaching data at some offset means decompressing everything before it in the stream. Capping offsets at one chunk (~16 MB of raw data, the max buffer size) means a reader can cache one decompressed chunk and still give random access to any file. The stream-assignment logic follows from this: put small files in the *same* stream so they benefit from a shared compression dictionary, but put big files in *separate* streams so they stay randomly accessible.
