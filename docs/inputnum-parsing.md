...
2. **Small factors of `N`** — a `factorize` sieve pass (§6) over `N` itself. When the factor list has no pseudo-factors and `p ∤ F`, the predicate is `mod(p) == 0` — the residue is computed from the factor *structure* (§5), never materializing a trial quotient. Reports `No small factors.`, or `<N> is prime.` if the sieve fully resolved it, or the `Small factors: …` list. A small factor here means the candidate is composite before any test starts.
