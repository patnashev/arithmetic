...
- **Why `_time_stage` exists** (answered upstream). It is *not* vestigial: the top-most Progress's `time_total()` must always be actual — in particular when `abort_flag` is set and `progress_save()` fires, the persisted `time_total` has to include whatever attached descendants contributed since the outer's own last `update()`, without waiting for one. `_time_stage` is that in-flight contribution. A possible alternative — a descendant's `update()` calling `_parent->update(progress_total(), op_count)` instead of poking `_time_stage` — might be a better design, but per upstream it needs investigation before anyone attempts it.
